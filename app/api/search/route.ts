...
// Add a custom wrapper for handling OpenRouter timeouts
const withTimeoutHandling = async (modelPromise: Promise<any>, timeoutMs = 30000) => {
    ...
    try {
        // Race the model promise against the timeout
...
export async function POST(req: Request) {
    ...
    // Add better error handling for OpenRouter requests
    try {
  ...
  `;
                        ...
                        // For OpenRouter models, add a timeout
                        if (selectedModel === 'scira-openchat' || selectedModel === 'scira-toppy') {
                            let hasTimedOut = false;
                            const timeoutId = setTimeout(() => {
                                hasTimedOut = true;
                                console.error(`OpenRouter request timed out for model ${selectedModel}`);
                                throw new Error(`OpenRouter request timed out for model ${selectedModel}`);
                        ...
                        const response = streamText({
                            ...
                            onError(event) {
                                console.log('Error: ', event.error);
                                if (selectedModel === 'scira-openchat' || selectedModel === 'scira-toppy') {
                                    // Log more detailed error info for OpenRouter errors
                                    console.error('OpenRouter error details:', JSON.stringify(event));
                                }
                            },
                        });
                        ...
                        // Special handling for OpenRouter models - fall back to default model
                        if (selectedModel === 'scira-openchat' || selectedModel === 'scira-toppy') {
                            console.warn(`Error with OpenRouter model ${selectedModel}, falling back to default model`);
                            
                            // Return a meaningful error message to the client
                            dataStream.append(`حدث خطأ مع نموذج OpenRouter. جاري التبديل إلى النموذج الافتراضي... يرجى إرسال طلبك مرة أخرى.`);
                            return; // Simply return without throwing
                        } else {
                            // For other models, just pass the error through
                            throw error;
                        }
                    }
                }
            ...
            // Similar improvements for the else branch
            ...
            return createDataStreamResponse({
                execute: async (dataStream) => {
                    try {
                        ...
                        // Add timeout handling for OpenRouter models
                        if (selectedModel === 'scira-openchat' || selectedModel === 'scira-toppy') {
                            let hasTimedOut = false;
                            const timeoutId = setTimeout(() => {
                                hasTimedOut = true;
                        ...
                        console.error("Stream execution error in part 2:", error);
